...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--with_answer", help="Option to include answers to input graphs.",
                        action="store_true")
    ...
    with open(f'{dir_}src{a}_train.txt', "w", encoding="utf-8") as outfile:
        outfile.write("\n".join(lin_graphs[:18989]))

    with open(f'{dir_}src{a}_test.txt', "w", encoding="utf-8") as outfile:
        outfile.write("\n".join(lin_graphs[18989:20989]))

    with open(f'{dir_}src{a}_val.txt', "w", encoding="utf-8") as outfile:
